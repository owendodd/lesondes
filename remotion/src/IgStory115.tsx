...
export interface IgStory115Props {
  durationSec?: number;
}

export function IgStory115({ durationSec: _d = 10 }: IgStory115Props) {
  ...
  return (
    <div style={outerStyle}>
      <OffthreadVideo src={staticFile("video/remotion/background2.mov")} style={bgStyle} muted playbackRate={0.8} />
      <div style={centerStack}>
        <Strip><p style={title}>Getting there<br />from Marseille</p></Strip>
        <Strip><p style={body}>Daily trains from Saint-Charles<br />to Cerbère (1 stop, ~4 hours)</p></Strip>
        <Strip><p style={body}>Drive  (~4 hours)</p></Strip>
      </div>
    </div>
  );
}
